...
layui.define(['jquery', 'table'], function (exports) {
    ...
    yutons_sug.prototype.render = function (opt) {
        ...
        if (opt.type == "sugTable") {
            ...
            //    opt.obj = this;
            //    getSug(opt);
            //    // console.info($.trim($(opt.obj).val()));
            //})
            var cpLock = true;
            $("#" + opt.id).on('input', function () {
                opt.obj = this;
                console.info("cplock:" + cpLock);
                if (cpLock) {
                    getSug(opt);
                    console.info($.trim($(opt.obj).val()));
                }
    ...
    function getSug(opt) {
        ...
        $.ajax({
            type: "get",
            url: opt.urlBak + "?cityId=" + cityId + "&text=" + kw,
            success: function (data) {
                let html = "";
                if (data.status == 1) {
                    layui.each(data.data.loupan, (index, item) => {
                        //console.info(item.name);
                        html +=
                            "<div class='item' style='padding: 3px 10px;cursor: pointer;margin-top:2px;overflow-x:auto;' onmouseenter='getFocus(this)' onClick='getCon(this);'>" +
                            item["name"] +
                            " <br/><span style='color:#c2c2c2'>" + item.addrEm + "</span >  </div > ";
